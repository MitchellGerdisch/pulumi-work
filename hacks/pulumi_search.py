...
# It uses the `properties` parameter to cause the API to return all the properties for the given resources.
# It uses the `query` parameter to enable limiting the search.
...
# HOW TO USE
# Define a query below you want to send if no query is given then ALL resources and ALL their properties will be returned.
# - HELPFUL HINT: Use Pulumi Cloud UI to figure out the query you want to send. The Search AI feature helps with this as well.
# - If you set `query=""` it will return all resources in the organization. (See note above about >10K resources.)
