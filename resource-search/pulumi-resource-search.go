...
.PARAMETER orgs
A comma-separated list of pulumi organization(s) to search in.
...
func main() {
	...
	orgs_param := flag.String("orgs", "", "Comma-separated list of Pulumi organization(s) to search in.")
	id := flag.String("id", "", "Resource id as it exists in the cloud provider.")

	flag.Parse()

	if (*access_token == "") || (*orgs_param == "") {
		fmt.Println("*************")
		fmt.Println("Missing access_token and/or orgs command line argument ...")
