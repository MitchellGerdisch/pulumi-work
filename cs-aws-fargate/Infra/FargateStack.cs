...
class FargateStack : Stack
{
    public FargateStack()
    {
        ...
        var taskCpu = config.Get("taskCpu") ?? "256";
        var taskMem = config.Get("taskMem") ?? "512";
        var desiredCount = config.GetInt32("desiredCount") ?? 3;
        ...
        var appRepo = new Ecr.Repository("app-repo", new()
        {
            ForceDelete = true,

        });
        var authToken = Ecr.GetAuthorizationToken.Invoke(new()
        {
            RegistryId = appRepo.RegistryId,
        });

        var image = new Docker.Image("my-app-image", new()
        {
            ...
            ImageName = appRepo.RepositoryUrl.Apply(repositoryUrl => $"{repositoryUrl}:latest"),
        ...
        //     Registry = new Docker.RegistryImage
        //     {
        //         Server = appRepo.RepositoryUrl,
