...
for resource in azure_resources:
    ...
    if resource_type in type_mappings:
        ...
        if num_resources_processed % num_resources_per_file == 0:
            ...
            bulk_import_file_name = f"{bulk_import_file_base}_{num_resources_processed//num_resources_per_file}.json"
            try:
                with open(bulk_import_file_name, 'w') as file:
                    file.write(bulk_resources_formatted)
            except Exception as e:
                print(f"An unexpected error occurred while writing {bulk_import_file_name}: {e}")
            else:
                print(f"Successfully wrote bulk import file to {bulk_import_file_name}.")
            # Reset the bulk_resources array
            bulk_resources = [] 
    else:
