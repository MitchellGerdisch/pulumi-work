...
ssh_key = tls.PrivateKey("sshKey",
    algorithm="RSA",
    rsa_bits=4096)

# Write the private key to a file
def write_private_key(private_key):
    with open("/tmp/wp_instance_private_key.pem", "w") as f:
        f.write(private_key)
    os.chmod("/tmp/wp_instance_private_key.pem", 0o600)

ssh_key.private_key_pem.apply(lambda privkey: write_private_key(privkey))

wordpress_keypair = aws.ec2.KeyPair("wordpress-keypair", public_key=ssh_key.public_key_openssh)
...
-u ec2-user \
-i '{public_ip},' \
--private-key {"/tmp/wp_instance_private_key.pem"} \
